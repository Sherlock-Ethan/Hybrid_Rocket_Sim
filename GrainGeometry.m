...
SlotD(1) = cosd(theta2(1))*SlotR(1) - cosd(theta1(1))...    % Full length of the slot (includes remainder)
           *(GrainID(1)/2);                                     
Perimeter(1) = SlotEnd(1)*8 + SlotD(1)*16 + PTip(1)*8;

% Surface Area
SA(1) = Perimeter(1) * GrainL;                              % m^2

% Crossectional Area
alpha(1) = theta2(1)*2;
alpha2(2) = theta1(1)*2;
CirSegTop(1) = (1/2)*(SlotR(1)^2)*(alpha(1) - sind(alpha(1)));         % ******************************************  Are alphas in correct units?
...
if n > 1
   % Driving Variables
   SlotW(n) = SlotW(n-1) + 2*RgrsPerStp; 
   ...
   % Check if the tip is still present
    ...
    theta2(n) = asind(SlotHW(n)/SlotR(n));                  
